...
def test_apply_update_success_launches_install_sh_detached():
    responses = {
        "rev-parse --is-inside-work-tree": (0, "true", ""),
        "fetch --tags --quiet origin main": (0, "", ""),
        "reset --hard origin/main": (0, "", ""),
        "describe --tags --always --dirty": (0, "v0.5", ""),
    }
    with patch.object(update_manager.system_tools, "find_binary", return_value="/usr/bin/git"), \
    ...
    assert result == {"success": True, "version": "v0.5"}
    mock_popen.assert_called_once()
    args, kwargs = mock_popen.call_args
    shell_command = args[0][-1]
    # the actual work happens in install.sh, not duplicated Python logic -
    # this only has to prove the right script gets launched, detached
    assert "install.sh" in shell_command
    assert update_manager.APP_DIR in shell_command
...
def test_apply_update_fetch_failure_does_not_launch_install(): 
    responses = {
        "rev-parse --is-inside-work-tree": (0, "true", ""),
        "fetch --tags --quiet origin main": (1, "", "network unreachable"),
    }
    ...
    assert result == {"success": False, "error_code": "update.fetch_failed"}
    mock_popen.assert_not_called()


def test_apply_update_reset_failure_does_not_launch_install():
    responses = {
        "rev-parse --is-inside-work-tree": (0, "true", ""),
        "fetch --tags --quiet origin main": (0, "", ""),
