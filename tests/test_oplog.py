...
class TestLogEvent(OplogTestCase):
    def test_records_success_event(self):
        entry = oplog.log_event("users", "create", "success", params={"username": "wieslaw"})
        self.assertEqual(entry["status"], "success")
        self.assertEqual(entry["category"], "users")
        self.assertEqual(entry["action"], "create")
        self.assertEqual(entry["params"], {"username": "wieslaw"})
        self.assertEqual(entry["message"], "")
        self.assertIn("timestamp", entry)

        events = oplog.list_events()
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]["params"]["username"], "wieslaw")

    def test_records_failure_with_message(self):
        oplog.log_event("shares", "create", "failure", params={"name": "test"}, message="testparm error: bad path")
        events = oplog.list_events()
    ...
    def test_unknown_status_treated_as_failure(self):
        oplog.log_event("users", "create", "weird")
        self.assertEqual(oplog.list_events()[0]["status"], "failure")

    def test_missing_params_defaults_to_empty_dict(self):
        oplog.log_event("certs", "generate", "success")
        self.assertEqual(oplog.list_events()[0]["params"], {})

    def test_ids_increment_and_persist_across_calls(self):
        e1 = oplog.log_event("users", "create", "success")
        e2 = oplog.log_event("users", "create", "success")
        self.assertEqual(e2["id"], e1["id"] + 1)

    def test_newest_first(self):
        oplog.log_event("users", "create", "success", params={"username": "first"})
        oplog.log_event("users", "create", "success", params={"username": "second"})
        events = oplog.list_events()
        self.assertEqual(events[0]["params"]["username"], "second")
        self.assertEqual(events[1]["params"]["username"], "first")
...
class TestMaxEntries(OplogTestCase):
    ...
    def test_oldest_dropped_once_cap_exceeded(self):
        ...
        for i in range(15):
            oplog.log_event("users", "create", "success", params={"n": i})
        events = oplog.list_events()
        self.assertEqual(len(events), 10)
        # newest first, so entry 14 should be present and entry 0 gone
        self.assertEqual(events[0]["params"]["n"], 14)
        ns = [e["params"]["n"] for e in events]
        self.assertNotIn(0, ns)
...
class TestClearEvents(OplogTestCase):
    def test_clear_removes_all_entries(self):
        oplog.log_event("users", "create", "success")
        oplog.log_event("users", "create", "success")
...
class TestListEventsTimeRange(OplogTestCase):
    def test_since_filters_out_older_events(self):
        with mock.patch("nas_monitor.oplog.datetime") as mock_dt:
            ...
            oplog.log_event("users", "create", "success", params={"username": "old"})

        with mock.patch("nas_monitor.oplog.datetime") as mock_dt:
            mock_dt.now.return_value.isoformat.return_value = "2026-01-02T10:00:00+00:00"
            mock_dt.fromisoformat = __import__("datetime").datetime.fromisoformat
            oplog.log_event("users", "create", "success", params={"username": "new"})

        events = oplog.list_events(since="2026-01-02T00:00:00+00:00")
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]["params"]["username"], "new")
