...
18. **Blokowanie dostępu, priorytet nad grupą** - ✅ zrobione. Wyrosło z
    pytania: czy "Brak dostępu" wprost dla użytkownika ma pierwszeństwo
    przed dostępem, który dostaje przez grupę? Odpowiedź: nie miał -
    dziś to naprawione, bo to była realna niespójność z zasadą
    **fail-safe defaults** (Saltzer i Schroeder, 1975 - domyślnie
    odmowa, jawne przyzwolenie musi wygrywać nad domyślnym). Dokładnie
    tak działają ACL w Windows/NTFS: jawny Deny zawsze bije Allow,
    niezależnie skąd to Allow pochodzi.
    - Nowy, **jawnie wybierany** poziom "Zablokowany (nawet przez
      grupę)" - celowo osobny od zwykłego "Brak dostępu" (który
      pozostaje neutralny i NIE blokuje dostępu z grupy - bo inaczej
      każdy niedotknięty wiersz w dialogu udziału cichcem blokowałby
      dostęp z grupy, rozwalając sens całego mechanizmu grup
      zbudowanego w poprzednim punkcie).
    - Renderowany przez `invalid users` w Sambie - potwierdzone wprost
      w dokumentacji Samby: "In the event that a user or group appears
      in both lists, the invalid users option takes precedence" -
      wygrywa zawsze, niezależnie od ścieżki dostępu.
    - `compute_effective_permission()` - jedna, autorytatywna funkcja
      licząca priorytet (Zablokowany > RO z dowolnego źródła > RW z
      dowolnego źródła > Brak), świadomie **nieoparta na zgadywaniu, jak
      Samba rozstrzyga konflikt** między wpisem indywidualnym a
      grupowym w `read list`/`write list` - dokumentacja Samby jest
      tu niejednoznaczna, więc zamiast ryzykować błędne założenie przy
      czymś bezpieczeństwo-krytycznym, ta funkcja **jest** źródłem
      prawdy, używanym identycznie po stronie renderowania configu i po
      stronie frontu (JS-owy port jeden do jednego), więc nigdy się nie
      rozjadą.
    - Front: etykieta przy każdym użytkowniku pokazuje teraz zawsze
      **efektywny** wynik (nie samą deklarację), przyciemniona gdy
      różni się od tego, co user sam wybrał dla siebie indywidualnie -
      np. indywidualne RW po cichu zawężone do RO przez bardziej
      restrykcyjną grupę. Brak indywidualnego wpisu (dostęp wyłącznie
      z grupy) świadomie **nie** liczy się jako "nadpisane", bo nie ma
      tam niczyjej własnej decyzji do zignorowania.
    - 8 nowych testów samej funkcji priorytetu (w tym oba kierunki
      konfliktu RW/RO), 5 kolejnych dla całego mechanizmu blokowania
      (nigdy nie dołącza do grupy dostępu, poprawny zapis/odczyt
      `invalid users`, brak fałszywego ostrzeżenia o braku hasła SMB
      dla zablokowanych, poprawna zmiana zablokowany→RW), zweryfikowane
      end-to-end w jsdom na dokładnie tym scenariuszu konfliktu.
